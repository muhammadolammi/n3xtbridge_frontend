...
export default function PublicInvoiceViewer() {
  ...
  useEffect(() => {
    ...
    const fetchInvoiceFromAPI = async () => {
      try {
        ...
        const res = await api.get(`/p/invoices/${id}`);
        ...
        const msg =
          err.response?.status === 404 ? "Record Not Found" : "Invalid Session";
        setError(msg);
      } finally {
        setLoading(false);
      }
  ...
  const handlePay = async (invoiceID: string) => {
    setLoading(true);
    try {
      // Grab token from URL search params
      const searchParams = new URLSearchParams(window.location.search);
      const token = searchParams.get("token");
      const checkoutUrl = await makePayment(invoiceID, token);
          ...
          <div className="p-6 md:p-16 print:p-0">
            ...
            <div className="flex flex-col md:flex-row justify-between mb-12 md:mb-16 gap-8">
              ...
              <div className="text-left md:text-right flex flex-col md:items-end gap-2">
                ...
                <div
                  className={`mt-2 inline-block px-4 py-1 rounded border text-[10px] font-black uppercase tracking-[0.2em] ${getStatusStyles(invoice.status)}`}
                >
                  {invoice.status}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-12 border-y border-gray-50 py-10">
              ...
              <div className="sm:text-right flex flex-col sm:justify-center">
                ...
                <p className="text-sm font-bold text-gray-900">
                  {new Date(invoice.created_at.Time).toLocaleDateString(
                    "en-US",
                    { dateStyle: "long" },
                  )}
                </p>
                ...
                <tbody className="divide-y divide-gray-50">
                  {invoice.items.map((item, i) => (
                    <tr key={i} className="print:break-inside-avoid">
                      ...
                      <td className="py-6 text-center text-sm text-gray-600">
                        {item.quantity}
                      </td>
              ...
              <div className="md:w-72 space-y-4">
                ...
                <div className="flex justify-between items-end pt-6 border-t-2 border-gray-900">
                  <div className="text-left">
                    <span className="block text-[10px] font-black uppercase tracking-tighter text-gray-400">
                      Total Payable
                    </span>
