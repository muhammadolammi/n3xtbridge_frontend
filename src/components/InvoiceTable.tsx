...
const InvoiceTable = ({
  ...
  const formatReminderDate = (dateString: string) => {
    if (
      ...
      return (
        <span className="text-gray-600 italic lowercase tracking-tight">
          Not sent
        </span>
      );
    }
    ...
    <div className="overflow-x-auto overflow-y-visible scrollbar-hide">
      <table className="w-full text-left min-w-[900px]">
        <thead className="bg-gray-300">
          <tr>
            <th className="px-6 py-5 text-[10px]  uppercase tracking-[0.2em] text-text">
              Invoice No.
            </th>
            <th className="px-6 py-5 text-[10px]  uppercase tracking-[0.2em] text-text">
              Quote
            ...
            <th className="px-6 py-5 text-[10px]  uppercase tracking-[0.2em] text-text text-right">
              Amount
            </th>
            <th className="px-6 py-5 text-[10px]  uppercase tracking-[0.2em] text-text">
              Last Reminder
        ...
        <tbody className="divide-y divide-[#22D3EE]">
          {loading ? (
            <tr>
              <td
                colSpan={7}
                className="p-20 text-center text-text font-bold italic tracking-widest animate-pulse"
              >
                Loading invoices...
              </td>
            </tr>
            ...
            <tr>
              <td
                colSpan={7}
                className="p-20 text-center text-text font-medium"
              >
                No invoices found.
              </td>
            </tr>
            ...
            invoices.map((inv, index) => {
              const quoteRef = inv.quote_id
                ? `Q-${inv.quote_id.slice(0, 8).toUpperCase()}`
                : "No Quote";
              return (
                <tr
                  key={inv.id}
                ...
                >
                  <td className="px-6 py-5  text-xs text-text">
                    <div className="flex items-center gap-2">
                      #{inv.invoice_number}
                      {inv.deleted_at && inv.deleted_at.Valid && (
                  ...
                  <td className="px-6 py-5">
                    <p className="text-sm font-italic text-text/80 tracking-tight">
                      {inv.customer_name}
                    </p>
                    <p className="text-[10px] text-text/70 uppercase font-mono">
                      {inv.customer_email}
                    </p>
                  </td>
                  <td className="px-6 py-5">
                    <span
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[9px] font-black uppercase tracking-widest ${STATUS_STYLES[inv.status.toLowerCase()] || STATUS_STYLES.default}`}
                    >
